...
void Vendaval::Editors::Scene::CreateEvent()
{
...
void Vendaval::Editors::Scene::RenderEvent()
{
	...
	const Rectangle sourceRect = {0.0f, 0.0f,
								  static_cast<float>(mViewportTexture.texture.width),
								  -static_cast<float>(mViewportTexture.texture.height)};
	rlImGuiImageRect(&mViewportTexture.texture, static_cast<int>(mViewportPanelSize.x),
					 static_cast<int>(mViewportPanelSize.y), sourceRect);
	...
	if (ImGui::IsWindowHovered())
	{
		...
		if (IsMouseButtonDown(cameraMoveButton))
		{
			...
			auto cameraLookAt =
				(Vec3(mCamera.target) - Vec3(mCamera.position)).Normalized();
			...
			auto cameraInput = Vec3(IsKeyDown(KEY_D) - IsKeyDown(KEY_A),
			...
			Vec3 worldMovement = (cameraLookAt * cameraInput.z) +
								 (cameraRight * cameraInput.x) +
								 (cameraUp * cameraInput.y);
			...
			SetMousePosition(ImGui::GetWindowPos().x + ImGui::GetWindowWidth() / 2,
...
void Vendaval::Editors::Scene::PostRenderEvent()
{
		...
		static_cast<int>(mViewportPanelSize.y) != mViewportTexture.texture.height)
	{
		...
		mViewportTexture = LoadRenderTexture(static_cast<int>(mViewportPanelSize.x),
											 static_cast<int>(mViewportPanelSize.y));
	}
}
